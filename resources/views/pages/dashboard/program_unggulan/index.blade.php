            ...
            <div class="grid w-full gap-5 px-10 mx-auto md:grid-cols-12">
                ...
                <div class="col-span-4 lg:text-right">
                    <div class="relative mt-0 md:mt-6">
                        <a href="{{ route('admin.program_unggulan.create') }}"
                        ...
                        <table class="w-full" aria-label="Table">
                            <thead>
                                <tr class="text-sm font-normal text-left text-gray-900 border-b border-b-gray-600">
                                    <th class="py-4 text-center" scope="">Judul</th>
                                    <th class="py-4 text-center" scope="">Logo</th>
                                    <th class="py-4 text-center" scope="">Action</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white">
                                @forelse ($program_unggulan as $item)
                                        <td class="px-1 py-5 text-sm">
                                            {{ $item->judul ?? '' }}
                                        </td>
                                        <td class="w-2/6 px-1 py-5">
                                            <div class="flex items-center text-sm">
                                                <div class="relative w-25 h-20 mr-3 rounded-full md:block">
                                                    @if (isset($item->logo) != null)
                                                        <img class="object-cover w-full h-full rounded"
                                                            src="{{ url(Storage::url($item->logo)) }}"
                                        ...
                                        <td class="w-2/6 px-1 py-5 text-sm text-center">
                                            <a href="{{ route('admin.program_unggulan.edit', $item->id) }}"
                                                class="px-4 py-2 mt-1 mr-2 text-center text-white rounded-xl bg-serv-button">Edit</a>

                                            <a href="{{ route('admin.program_unggulan.destroy', $item->id) }}"
                                            ...
                                            <form action="{{ route('admin.program_unggulan.destroy', $item->id) }}"
